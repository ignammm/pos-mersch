<div class="p-4">
    ...
    <div x-data="{ mostrar: false }" x-init="@this.on('ingreso-create', () => {
        mostrar = true;
    ...
    <x-modal wire:model.live="mostrarModalDuplicados">
        <div class="p-6 bg-gray-50 min-h-screen">
            ...
            @if ($coincidenciasArt && $coincidenciasArt->isNotEmpty())
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                    @foreach ($coincidenciasArt as $art)
                        <div
                            class="bg-white rounded-xl shadow-md hover:shadow-lg transition p-5 flex flex-col justify-between">
                            <div class="space-y-2">
                                <p class="text-gray-700"><span class="font-semibold">Artículo:</span>
                                    {{ $art->articulo ?? '' }}</p>
                                <p class="text-blue-800"><span class="font-semibold">Marca:</span>
                                    {{ $art->marca ?? '' }}</p>
                                <p class="text-gray-700"><span class="font-semibold">Rubro:</span>
                                    {{ $art->rubro ?? '' }}</p>
                                <p class="text-gray-700"><span class="font-semibold">Precio:</span>
                                    {{ $art->precio ?? '' }}</p>
                            </div>
                            <div class="mt-4 flex justify-end">
                                <button wire:click="confirmarSeleccionArt({{ $art->id }})"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach ($coincidenciasRef as $ref)
                        <div
                            ...
                            <div class="mt-4 flex justify-end">
                                <button wire:click="confirmarSeleccionRef({{ $ref->id }})"
                                    class="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition">
                                    Seleccionar
                                </button>
                            </div>
            ...
            <div class="flex justify-end mt-8">
                <button wire:click="$set('mostrarModalDuplicados', false)"
